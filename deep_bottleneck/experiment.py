...
NUM_THREADS = 3
K.set_session(tf.Session(config=tf.ConfigProto(intra_op_parallelism_threads=NUM_THREADS, inter_op_parallelism_threads=NUM_THREADS)))
...
def make_plotters(plotters, _run, dataset):
    ...
    for plotter in plotters:
        plotter_object = importlib.import_module(plotter[0]).load(
            _run, dataset, *plotter[1]
        )
...
@ex.capture
def make_callbacks(callbacks, data, batch_size, _run, file_dump_train, file_dump_test):
    callback_objects = []

    callback_objects.append(
        ActivityLogger(
            data.train.examples, do_save_func=do_report, file_dump=file_dump_train
        )
    )

    callback_objects.append(
        ActivityLogger(
            data.test.examples, do_save_func=do_report, file_dump=file_dump_test
        )
    )
...
def conduct(epochs, batch_size, n_runs, _run, steps_per_epoch=None, plot_median=False):
    ...
    for run_id in range(n_runs):
        ...
        model = load_model(
            input_size=data.train.examples.shape[1], output_size=data.n_classes
        )
        ...
        callbacks = make_callbacks(
            data=data, file_dump_train=file_dump_train, file_dump_test=file_dump_test
        )

        if steps_per_epoch == None:
            model.fit(
                x=data.train.examples,
                y=data.train.one_hot_labels,
                verbose=0,
                batch_size=batch_size,
                epochs=epochs,
            ...
            model.fit_generator(
                generator(data.train.examples, data.train.one_hot_labels, batch_size),
                steps_per_epoch=steps_per_epoch,
                epochs=epochs,
                validation_data=generator(
                    data.test.examples, data.test.one_hot_labels, batch_size
                ),
        ...
        measures_train = estimator.compute_mi(data.train, file_dump=file_dump_train)
        measures_train["run"] = run_id
        measures_all_runs_train.append(measures_train)

        measures_test = estimator.compute_mi(data.test, file_dump=file_dump_test)
    ...
    if plot_median:
        mi_mean_over_runs_train = measures_all_runs_train.groupby(
            ["epoch", "layer"]
        ).median()
        mi_mean_over_runs_test = measures_all_runs_test.groupby(
            ["epoch", "layer"]
        ).median()
    else:
        mi_mean_over_runs_train = measures_all_runs_train.groupby(
    ...
    measures_summary_train = {
        "measures_all_runs": measures_all_runs_train,
        "mi_mean_over_runs": mi_mean_over_runs_train,
        "activations_summary": file_dump_train,
    }

    measures_summary_test = {
        "measures_all_runs": measures_all_runs_test,
